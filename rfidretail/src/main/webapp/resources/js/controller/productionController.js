app.controller('productionListController', ['$scope', '$http', '$location', '$rootScope', '$window', '$filter', '$uibModal', function ($scope, $http, $location, $rootScope, $window, $filter, $uibModal) {
	...
	$http.get('/company/getCompanyList').success(
		function(data) {
			...
			httpGetList("/production", param, $scope, $http, true);
			...
			/*
			$http.get('/production/countAll' + param).success(
				function(data) {
	...
	$scope.goPage = function(page){
		...
		httpGetList("/production", param, $scope, $http);
...
//		
//		$location.url("/production/productionDetail?seq=" + obj.productionStorageSeq);
	...
	$scope.inspection = function(){
		...
		angular.forEach($scope.checkList, function(value, key) {
			if($scope.checkList[key].nonCheckAmount > 0){
				amountFlag = true;
			}
		});
...
app.controller('productionDetailPopController', ['$scope', '$http', '$location', '$rootScope', '$interval', '$routeParams', 'verificationFactory', '$filter', '$window', '$uibModal', '$uibModalInstance', 'obj', function ($scope, $http, $location, $rootScope, $interval, $routeParams, verificationFactory, $filter, $window, $uibModal, $uibModalInstance, obj) {
	    			...
	    			$http.get("/productionRfidTag/findTag?seq=" + seq).success(function(data) {
						...
						$http({
					    	...
					    	modalOpen($uibModal,  "전체 재발행요청 완료되었습니다.", function() {
								$(location).attr("href", '#/production/productionList');
							});
					    }).error(function(){
					    	modalOpen($uibModal,  "에러 발생");
					    });
