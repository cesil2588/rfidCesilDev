app.controller('storeListController', ['$scope', '$http', '$location', '$rootScope', '$window', '$filter', '$uibModal', function ($scope, $http, $location, $rootScope, $window, $filter, $uibModal) {
	
	$scope.ajaxFinish = false;
	
	$scope.search = initSearch("style", "storeStorageSeq", "desc", angular.fromJson($window.sessionStorage.getItem("current")));
	...
	$http.get('/company/getCompanyList').success(
		function(data) {
			...
			httpGetList("/store", param, $scope, $http, true);
			
			$http.get('/store/select/productYy' + param).success(
				function(data) {
			...
			/*
			$http.get('/store/countAll' + param).success(
				function(data) {
	...
	$scope.goPage = function(page){
		...
		httpGetList("/store", param, $scope, $http);
...
//		
//		$location.url("/store/storeDetail?seq=" + obj.storeStorageSeq);
		
		 detailPopOpen($uibModal, obj, "/store/storeDetailPop", "storeDetailPopController", "xxlg");
	};
	...
	$scope.selectItem = function(item, flag){
		...
		$http.get('/store/select/' + $scope.search.urlFlag + param).success(
			function(data) {
				if(flag == "productYy"){
...
app.controller('storeDetailPopController', ['$scope', '$http', '$location', '$rootScope', '$interval', '$routeParams', 'verificationFactory', '$filter', '$window', '$uibModal', '$uibModalInstance', 'obj', function ($scope, $http, $location, $rootScope, $interval, $routeParams, verificationFactory, $filter, $window, $uibModal, $uibModalInstance, obj) {
	...
	var seq = obj.storeStorageSeq;
	
	var param = generateParam($scope.search);
	
	$http.get('/store/' + seq).success(
		function(data) {
	...
	if($rootScope.testMode){
		httpGetList("/storeRfidTag/" + seq, param, $scope, $http);
	}
	...
	*/
	...
	$scope.tagStat = function(obj){
		...
		param = generateParam($scope.search);
		httpGetList("/storeRfidTag/" + seq, param, $scope, $http, true);
	};
