...
<div class="fade show starter-template" ng-show="authenticated && ajaxFinish">
	<div class="container-fluid">
		<div class="body-contents" ng-class="sidebarFlag ? 'sidebar-true' : 'sidebar-false'">
			<h2><a href="" ng-click="reload()">배치 예약 목록</a></h2>
				...
				<div class="form-row"> 
	          	...
	          	<div class="form-row">
		          	<div class="form-group form-inline">
		          		작업 명 : 
		          		&nbsp;&nbsp;
		          		<select class="custom-select" style="width:200px;" ng-model="search.type">
							<option value="all" ng-selected="true">전체 보기</option>
							<option value="1">발행 업로드 배치</option>
							<option value="4">재발행 업로드 배치</option>
							<option value="3">일괄 생산 태그 입고 배치</option>
						</select>
		          		&nbsp;&nbsp;
		          		상태 :
		          		&nbsp;&nbsp; 
		          		<select class="custom-select" style="width:120px;" ng-model="search.status">
							<option value="all" ng-selected="true">전체 보기</option>
							<option value="SCHEDULE">미완료</option>
			...
			<table class="table table-striped table-bordered table-hover text-center custom-align-middle">
				<thead>
					<tr class="pointer">
						<th>작업 명</th>
						<th ng-click="sort('status')">상태</th>
						<th ng-click="sort('regUserInfo.userId')">등록자</th>
						<th ng-click="sort('scheduleDate')">등록일시</th>
						<th ng-click="sort('startDate')">시작일시</th>
						<th ng-click="sort('endDate')">종료일시</th>
					</tr>
				</thead>
				<tbody>
					<tr ng-repeat="obj in list" ng-click="detail(obj)" class="pointer" ng-if="list.length > 0">
						<td ng-switch="obj.type">
							<span ng-switch-when="1">발행 업로드 배치</span>
							<span ng-switch-when="3">일괄 생산 태그 입고 배치</span>
							<span ng-switch-when="4">재발행 업로드 배치</span>
						</td>
						<td ng-switch="obj.status">
							<span ng-switch-when="SCHEDULE">미완료</span>
							<span ng-switch-when="COMPLETE">완료</span>
						</td>
						<td>{{obj.regUserInfo.userId}}</td>
						<td>{{obj.scheduleDate | date:'yyyy-MM-dd HH:mm:ss'}}</td>
						<td>{{obj.startDate | date:'yyyy-MM-dd HH:mm:ss'}}</td>
						<td>{{obj.endDate | date:'yyyy-MM-dd HH:mm:ss'}}</td>
					</tr>
					<tr>
