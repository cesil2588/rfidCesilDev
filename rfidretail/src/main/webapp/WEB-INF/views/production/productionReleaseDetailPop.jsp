...
<html ng-app="myApp">
	<div class="modal-header">
		<h3 class="modal-title" id="modal-title">출고 박스 상세</h3>
	</div>
							...
							<table class="table table-striped table-bordered table-hover text-center custom-align-middle">
								<thead>
									<tr class="pointer">
										...
										<th ng-click="sort('boxInfo.createDate')">날짜</th>
								...
								<tbody>
									<tr>
										...
										<td><span code-name-fix pcode="10001" code="{{data.boxInfo.type}}"></span></td>
										<td>{{data.boxInfo.createDate}}</td>
										...
										<td>{{data.confirmYn}}</td>
										<td>
											<span ng-if="data.confirmDate">
												{{data.confirmDate | date:'yyyy-MM-dd'}}<br/>
												{{data.confirmDate | date:'HH:mm:ss'}}<br/>
											</span>
											<span ng-if="!data.confirmDate">
												-
											</span>
										...
										<td>
											<span ng-if="data.confirmYn == 'N' && data.completeYn == 'N'">출고 예정</span>
											<span ng-if="data.confirmYn == 'Y' && data.completeYn == 'N'">출고 확인</span>
											<span ng-if="data.confirmYn == 'Y' && data.completeYn == 'Y'">출고 완료</span>
										</td>
