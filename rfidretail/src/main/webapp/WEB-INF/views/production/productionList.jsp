...
<div class="fade show starter-template" ng-show="authenticated && ajaxFinish">
	<div class="container-fluid">
		<div class="body-contents" ng-class="sidebarFlag ? 'sidebar-true' : 'sidebar-false'">
			<h2><a href="" ng-click="reload()">생산태그정보 목록</a></h2>
			...
			<table class="table table-striped table-bordered table-hover text-center custom-align-middle" ng-if="list.length > 0">
				...
				<tbody>
					<tr class="pointer">
						...
						<td>{{countAll.allReleaseAmount | number:0}}</td>
						<td>{{countAll.allReissueAmount | number:0}}</td>
						<td>{{countAll.allDisuseAmount | number:0}}</td>
						<td>{{countAll.allReturnNonCheckAmount | number:0}}</td>
						<td>{{countAll.allReturnAmount | number:0}}</td>
					</tr>
				</tbody>
			</table>
			-->
			...
			<table class="table table-striped table-bordered table-hover text-center custom-align-middle">
				<thead>
					<tr class="pointer">
						<th ng-click="sort('bartagMaster.customerName')">업체명</th>
						...
						<th ng-click="sort('stockAmount')">재고 수량</th>
						<th ng-click="sort('releaseAmount')">출고 수량</th>
						<th ng-click="sort('reissueAmount')">재발행요청 수량</th>
						<th ng-click="sort('disuseAmount')">폐기 수량</th>
						<th ng-click="sort('returnNonCheckAmount')">반품미검수 수량</th>
						<th ng-click="sort('returnAmount')">반품 수량</th>
				...
				<tbody>
					<tr ng-repeat="production in list" class="pointer" ng-if="list.length > 0">
						...
						<td ng-click="detail(production)">{{production.bartagMaster.detailProductionCompanyName.trim != '' ? production.bartagMaster.detailProductionCompanyName : '-'}}</td>
						...
						<td ng-click="detail(production)">{{production.totalAmount | number:0}}</td>
						...
						<td ng-click="detail(production)">{{production.releaseAmount | number:0}}</td>
						<td ng-click="detail(production)">{{production.reissueAmount | number:0}}</td>
						<td ng-click="detail(production)">{{production.disuseAmount | number:0}}</td>
						<td ng-click="detail(production)">{{production.returnNonCheckAmount | number:0}}</td>
						<td ng-click="detail(production)">{{production.returnAmount | number:0}}</td>
						<td ng-click="detail(production)">{{production.regDate | date:'yyyy-MM-dd'}}<br />
							{{production.regDate | date:'HH:mm:ss'}}</td>
					</tr>
					<tr>
						<td colspan="16" class="text-center" ng-if="list.length == 0">
							No Data
						</td>
					</tr>
				</tbody>
			...
			<div class="pull-right">
				<!-- <button ng-click="inspection()" class="btn btn-lg btn-primary">선택한 스타일 입고검수</button> -->
			</div>
		</div>
	</div>
</div>
