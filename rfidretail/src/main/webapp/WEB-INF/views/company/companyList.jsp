...
<div class="fade show starter-template" ng-show="authenticated && ajaxFinish">
	<div class="container-fluid">
		<div class="body-contents" ng-class="sidebarFlag ? 'sidebar-true' : 'sidebar-false'">
			<h2><a href="" ng-click="reload()">업체 목록</a></h2>
			<div class="card card-body" style="margin-bottom:5px;"> 
				<div class="form-inline">
					...
					<select class="custom-select" style="width:140px;margin-right:5px;" ng-model="search.type">
						...
						<option ng-repeat="type in typeList.codeInfo" value="{{type.codeValue}}">{{type.name}}</option>
					</select>
					&nbsp;&nbsp;
					업체명 : 
					&nbsp;&nbsp;
					<input type="text" ng-model="search.companyInfo" placeholder="업체명 입력" uib-typeahead="company as company.name for company in companyList | filter:{name:$viewValue}" class="form-control" typeahead-show-hint="true" typeahead-min-length="0">
					&nbsp;&nbsp;
			...
			<table class="table table-striped table-bordered table-hover text-center custom-align-middle">
				<thead>
					<tr class="pointer">
						...
						<th ng-click="sort('code')">업체코드</th>
						<th ng-click="sort('customerCode')">ERP 거래처코드</th>
						<th ng-click="sort('type')">업체타입</th>
						<th ng-click="sort('telNo')">전화번호</th>
						<th ng-click="sort('role')">권한</th>
						<th ng-click="sort('useYn')">사용여부</th>
						<th ng-click="sort('erpYn')">ERP여부</th>
						<th ng-click="sort('regDate')">등록일</th>
						<th ng-click="sort('updDate')">수정일</th>
					</tr>
				</thead>
				<tbody>
					<tr ng-repeat="company in list" ng-click="detail(company)" class="pointer" ng-if="list.length > 0">
						<td>{{company.name}}</td>
						<td>{{company.code ? company.code : '-'}}</td>
						<td>{{company.customerCode}}</td>
						<td><span code-name pcode="10002" code="{{company.type}}"></span></td>
						<td>{{company.telNo ? company.telNo : '-'}}</td>
						<td><span code-name pcode="10003" code="{{company.roleInfo.role}}"></span></td>
						<td>{{company.useYn}}</td>
						<td>{{company.erpYn}}</td>
						<td>
							<span ng-if="company.regDate">
								{{company.regDate | date:'yyyy-MM-dd'}}<br />
								{{company.regDate | date:'HH:mm:ss'}}
							</span>
							<span ng-if="!company.regDate">
								-
							</span>
						...
						<td>
							<span ng-if="company.updDate">
								{{company.updDate | date:'yyyy-MM-dd'}}<br />
								{{company.updDate | date:'HH:mm:ss'}}
							</span>
							<span ng-if="!company.updDate">
								-
							</span>
					...
					<form class="form-inline" ng-submit="clickSearch()">
						<div class="form-group">
							<select class="custom-select" ng-model="search.option" style="width:100px;"> 
								<option value="telNo" ng-selected="true">전화번호</option>
							</select>&nbsp;
