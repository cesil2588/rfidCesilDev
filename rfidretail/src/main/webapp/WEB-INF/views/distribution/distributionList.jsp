	...
	<div class="container-fluid">
		<div class="body-contents" ng-class="sidebarFlag ? 'sidebar-true' : 'sidebar-false'">
			...
			<table class="table table-striped table-bordered table-hover text-center custom-align-middle">
				<thead>
					<tr class="pointer">
						<th ng-click="sort('productionStorage.bartagMaster.customerName')">생산 업체명</th>
						<th ng-click="sort('productionStorage.bartagMaster.productYy')">연도</th>
						<th ng-click="sort('productionStorage.bartagMaster.productSeason')">시즌</th>
						<th ng-click="sort('productionStorage.bartagMaster.style')">스타일</th>
						<th ng-click="sort('productionStorage.bartagMaster.color')">컬러</th>
						<th ng-click="sort('productionStorage.bartagMaster.size')">사이즈</th>
						<th ng-click="sort('totalAmount')">총 수량</th>
						<th ng-click="sort('stat1Amount')">입고예정 수량</th>
						<th ng-click="sort('stat2Amount')">입고 수량</th>
						<th ng-click="sort('stat3Amount')">출고 수량</th>
						<th ng-click="sort('stat4Amount')">재발행대기 수량</th>
						<th ng-click="sort('stat5Amount')">재발행요청 수량</th>
						<th ng-click="sort('stat6Amount')">반품 수량</th>
						<th ng-click="sort('stat7Amount')">폐기 수량</th>
						<th ng-click="sort('regDate')">등록일</th>
					</tr>
				</thead>
				<tbody>
					<tr ng-repeat="distribution in list" ng-click="detail(distribution)" class="pointer" ng-if="list.length > 0">
						<td>{{distribution.productionStorage.bartagMaster.productionCompanyInfo.name}}</td>
						<td>{{distribution.productionStorage.bartagMaster.productYy}}</td>
						<td>{{distribution.productionStorage.bartagMaster.productSeason}}</td>
						<td>{{distribution.productionStorage.bartagMaster.style}}</td>
						<td>{{distribution.productionStorage.bartagMaster.color}}</td>
						<td>{{distribution.productionStorage.bartagMaster.size}}</td>
						<td>{{distribution.totalAmount | number:0}}</td>
						<td>{{distribution.stat1Amount | number:0}}</td>
						<td>{{distribution.stat2Amount | number:0}}</td>
						<td>{{distribution.stat3Amount | number:0}}</td>
						<td>{{distribution.stat4Amount | number:0}}</td>
						<td>{{distribution.stat5Amount | number:0}}</td>
						<td>{{distribution.stat6Amount | number:0}}</td>
						<td>{{distribution.stat7Amount | number:0}}</td>
						<td>{{distribution.regDate | date:'yyyy-MM-dd'}}<br />
							{{distribution.regDate | date:'HH:mm:ss'}}</td>
					</tr>
					<tr>
