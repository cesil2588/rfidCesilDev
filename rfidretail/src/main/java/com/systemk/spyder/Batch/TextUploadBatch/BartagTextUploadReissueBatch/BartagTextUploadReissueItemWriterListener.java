...
@Component
public class BartagTextUploadReissueItemWriterListener implements ItemWriteListener<BatchTrigger>{
	
	private static final Logger log = LoggerFactory.getLogger(BartagTextUploadReissueItemWriterListener.class);
	
	private BatchTriggerRepository batchTriggerRepository;
	
	private UserNotiService userNotiService;
	
	public BartagTextUploadReissueItemWriterListener(BatchTriggerRepository batchTriggerRepository,
	...
	public void afterWrite(List<? extends BatchTrigger> items) {
		...
		for(BatchTrigger trigger: items){
			...
			// 배치 트리거 사용자 noti
			userNotiService.save("배치 업로드 작업이 완료되었습니다.", trigger.getRegUserInfo(), "textUploadReissueBatch", Long.valueOf(0));
		}
	}
	...
	public void onWriteError(Exception exception, List<? extends BatchTrigger> items) {
		for(BatchTrigger trigger: items){
			...
			// 배치 트리거 사용자 noti
			userNotiService.save("배치 업로드 작업이 실패하였습니다.", trigger.getRegUserInfo(), "textUploadReissueBatch", Long.valueOf(0));
		}
