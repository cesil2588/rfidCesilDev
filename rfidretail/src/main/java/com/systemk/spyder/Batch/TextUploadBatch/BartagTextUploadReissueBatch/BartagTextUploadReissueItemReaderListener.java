...
@Component
public class BartagTextUploadReissueItemReaderListener implements ItemReadListener<BatchTrigger>{
	
	private static final Logger log = LoggerFactory.getLogger(BartagTextUploadReissueItemReaderListener.class);
	
	private BatchTriggerRepository batchTriggerRepository;
	
	private UserNotiService userNotiService;
	
	public BartagTextUploadReissueItemReaderListener(BatchTriggerRepository batchTriggerRepository,
	...
	public void afterRead(BatchTrigger trigger) {
		...
		// 배치 트리거 사용자 noti
		userNotiService.save("배치 업로드 작업이 시작 되었습니다.", trigger.getRegUserInfo(), "textUploadReissueBatch", Long.valueOf(0));
	}
